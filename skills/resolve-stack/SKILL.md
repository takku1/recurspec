---
name: resolve-stack
description: Decide what actually implements a component - buy a managed service, adopt a library, wrap one in an adapter, or build it - and emit a full Technology Resolution block with alternatives, exit cost, and cost model. Biases hard toward existing services and libraries over custom code. Use for one component, an existing spec node, or to re-open a stale decision.
---

# Resolve Stack

Answer, for one component: **what will actually implement this?** Produces a §8
Technology Resolution block backed by a real survey.

Use when:

- A spec node says what a component *does* but not what it *is built from*.
...
For a whole tree, use `/recursive-spec` — it runs this gate at every node.
Criteria: `docs/process/technology-resolution.md`.
...
sibling nodes. If the gap is substantial, say so — the component probably needs
decomposing via `/recursive-spec`, split at the boundary between what is bought and what
remains.
