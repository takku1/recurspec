...
One line from a flat plan, run through the [decomposition loop](../process/decomposition-loop.md)
and the [technology resolution gate](../process/technology-resolution.md).
...
> governs [research/foundation.md](../research/foundation.md) applies to §8 entries.
...
```
/recursive-spec    # drive the loop from a goal down to terminal nodes
/resolve-stack     # run the gate on one existing node, or re-open a stale §8
```

Then claim leaves from `.scratch/wayfinder-map/MAP.md` and implement via
[`/dual-loop`](../../skills/dual-loop/SKILL.md).
