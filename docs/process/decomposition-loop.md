...
Component contract: [architecture/technology-resolver/SYSTEM.md](../architecture/technology-resolver/SYSTEM.md) ·
Resolution criteria: [technology-resolution.md](./technology-resolution.md) ·
...
| **Reinvention by omission** | Nobody asked "does this exist already?", so a `users` table with hand-rolled password hashing gets written — the single most solved, highest-liability component in the list |
...
[technology-resolution.md](./technology-resolution.md). The short version: **BUILD is the
...
- it is uniformly BUILD but too large for one session (split by interface seam, the
  classic rule from [multi-signal-reconciler](./multi-signal-reconciler.md)).
...
3. **Distinct failure mode test.** If a proposed child cannot fail independently of its
   siblings, it is not a component — it is a step in a procedure. Do not give it a node.
...
| Process incompleteness | [open-work.md](../open-work.md) |
...
[examples/login-decomposition.md](../examples/login-decomposition.md).

---

## Relationship to the rest of RSS

| This loop | Feeds |
|-----------|-------|
| Terminal leaves + §6/§7 seams | `/dual-loop` strategy packets |
| DEFER nodes | Wayfinder Type B research tickets |
| §8 decisions | Back-Channel A: a dependency that drifts from its spec is code drift |
| Alternatives + exit cost | ADR history when a vendor is later swapped |

The loop runs **forward** (vision → leaves). The back-channels run **backward**
(reality → blueprint). A §8 entry whose pinned version no longer matches the lockfile is
exactly the kind of drift [`/reconcile-spec`](../../skills/reconcile-spec/SKILL.md) exists
to catch.
