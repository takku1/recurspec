# Multi-Signal Structural Reconciler

Back-Channel A detail. Component contract: [architecture/reconciler/SYSTEM.md](../architecture/reconciler/SYSTEM.md). Incomplete work: OW-02, OW-13 in [open-work.md](../open-work.md).
...
```mermaid
graph TD
    S1[Signal A: Code Drift] --> Agent[Reconciler Observer]
...
Signal D (metric drift) is empirical — owned by Measurement Harness + Back-Channel B; tracked as OW-13 until implemented.
...
3. Do not treat reconcile as a substitute for measurement (Back-Channel B).  
