...
Loop context: [decomposition-loop.md](./decomposition-loop.md) §3 ·
...
Some components carry obligations that transfer to a vendor when you buy them. This is
frequently the entire argument on its own.

| Component | Obligation you take on by building |
|-----------|-----------------------------------|
...
| Trigger | Signal |
|---------|--------|
| Pinned version no longer matches the lockfile | Back-Channel A code drift |
