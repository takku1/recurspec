...
correct answer (Spec Engine / Technology Resolver), or run state (job-store).
...
- **[Ubiquitous]** A worker SHALL receive a packet and SHALL NOT read the tree directly.
  - `EvidenceStage:` Unknown — the packet is the only abstraction barrier that makes
...
- **[Conditional]** IF a worker produced the node under review THEN THE SYSTEM SHALL NOT
  ...
  - `EvidenceStage:` Observed — maker ≠ checker ([dual-loop](../../../../skills/dual-loop/SKILL.md));
...
- **ADR-003:** Results are structured, not prose. The pool returns fields the Spec Engine
...
- **Harness:** `components/worker-pool/measure.sh`
- **Backpressure:** `components/worker-pool/checks.sh`
- **Telemetry:** per-worker `{node_id, phase, tier, tokens_in, tokens_out, ms, outcome}`
- **Branching:** worktree hypothesis; keep only on checks pass ∧ no latency regression ∧
...
- **Fit gap:** the SDK does not know RSS's phases, tiering policy, or budget rule. That gap
