...
**Does not own:** the loop's judgment (Stack Resolver), contract structure or
validation (Contract Engine), ticket publication (Frontier Adapter), or keep/revert
measurement (Evaluation Gate). The Runner never decides *what the answer is* — only
...
|-------|-----------|-------|
| Procurement pruning | BUY/ADOPT terminates a subtree unwritten | Stack Resolver (existing) |
...
| Schema-first checks | Structural rules cost zero tokens | context-packer / Contract Engine |
