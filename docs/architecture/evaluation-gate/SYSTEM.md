...
- **[Ubiquitous]** The Evaluation Gate SHALL compare a candidate against the trunk BKS and SHALL promote a new baseline only through explicit `--record-baseline` after merge.
  - `EvidenceStage:` Sampled (`find_baseline` and baseline-promotion unit coverage); production automation OW-05
- **[Event-driven]** WHEN `checks.sh` fails THE SYSTEM SHALL block keep regardless of primary metric improvement.
  - `EvidenceStage:` Sampled (`test_runner_logs_negative_patterns_and_enforces_total_attempt_ceiling`)
- **[Conditional]** IF telemetry self-contradiction is detected THEN THE SYSTEM SHALL flag the instrument broken and halt Empirical Feedback blueprint updates.
  - `EvidenceStage:` Sampled (legacy and multi-metric contradiction tests)
- **[State-driven]** WHILE a candidate branch is active THE SYSTEM SHALL isolate mutations from the main blueprint branch.
  - `EvidenceStage:` Unknown (policy specified; production worktree lifecycle remains OW-05) · *Research:* BranchBench / autoresearch branch–mutate–evaluate ([foundation](../../research/foundations.md) §4)
- **[Conditional]** IF a metric omits `tier` THEN THE SYSTEM SHALL treat it as `hard_gate`; `hard_gate`, `target`, and `optimization` regressions or unknown comparisons SHALL block keep, while `observation` SHALL never block.
  - `EvidenceStage:` Sampled (`resolve_tier` / `evaluate_candidate` unit coverage)
- **[Event-driven]** WHEN any candidate is reverted THE SYSTEM SHALL append one branch-scoped Negative Pattern and SHALL surface prior patterns before the next repair pass.
  - `EvidenceStage:` Sampled (runner integration coverage)
- **[Conditional]** IF a branch reaches 5 consecutive reverts or 8 total reverts THEN THE SYSTEM SHALL return `ESCALATE` (exit code 3) instead of authorizing another automatic repair.
  - `EvidenceStage:` Sampled (streak, branch-scope, total-ceiling, and runner integration coverage)
